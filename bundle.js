...
/***/ function(module, exports, __webpack_require__) {
	...
	riot.tag2('app', '<header> <div layout layout-align="start start"> <img width="175" src="img/flexmaid.png" alt=""> <div> <h1>Flex Maid</h1> <span> At your service </span> </div> </div> </header> <section> <div layout="column" layout-align="center"> <form layout layout-align="space-around start"> <input type="text" onkeyup="{changeName}" name="label" value="{breakpoint.label}"> {breakpoint.pixel}px <input type="submit" onclick="{add}" value="add"> </form> <div class="slider" name="pixelSlider"></div> </div> </section> <section class="breakpoints"> <breakpoint each="{breakpoint, index in breakpoints}" if="{breakpoint.active}" layout="row" layout-align="space-between" onclick="{select}" riot-style="background-color: hsl(330, 50%, {50+(40/breakpoints.length)*index}%)" label="{breakpoint.label}" pixel="{breakpoint.pixel}"> </breakpoint> </section>', '', '', function (opts) {
	    ...
	    this.switchBreakpoint = function (breakpoint) {
	        _this.breakpoint = breakpoint;
	        debugger;
	        _this.pixelSlider.noUiSlider.set(breakpoint.pixel);
	    };

	    this.add = function (e) {
	        var _this2 = this;

	        var equallyLabeledItems = this.breakpoints.filter(function (item) {
	            return item.label === _this2.breakpoint.value;
	        });
	        var pixel = this.breakpoint.pixel;
	        if (!equallyLabeledItems.length && this.label.value.length) {
	            var breakpointPushIndex = 0;
	            this.breakpoints.filter(function (item, index) {
	                if (pixel >= item.pixel) {
	    ...
	    this.on('mount', function () {
	        _this.noUiSlider.create(_this.pixelSlider, {
	            start: [_this.pixel],
	            range: {
	                'min': 320,
	                'max': 1920
	        ...
	        _this.switchBreakpoint(_this.breakpoints[3]);

	        _this.pixelSlider.noUiSlider.on('update', function () {
	            _this.trigger('sliderUpdated');
	        });
	    });

	    this.on('sliderUpdated', function () {
	        _this.breakpoint.pixel = _this.pixelSlider.noUiSlider.get() | 0;
	        console.log(_this.breakpoints);
	        _this.breakpoints.sort(function (previous, next) {
	            return previous.pixel - next.pixel;
	        });
	        _this.update();
	    });

	    this.on('remove', function (breakpoint) {
	        _this.breakpoints = _this.breakpoints.filter(function (bp) {
...
/***/ function(module, exports, __webpack_require__) {
	...
	riot.tag2('breakpoint', '<div layout layout-align="space-between"> <div> <span onclick="{remove}">&times;</span> </div> <div> <span>{opts.label} - {opts.pixel}</span> </div> </div>', '', '', function (opts) {
